...
% Define your datasets:your dataset consists now of X1;X2 and Tnew. Draw 3 (independent) samples of 1 000
% points each. Use them as the training set, validation set, and test set, respectively. 
% Motivate the choice of the datasets. 

% training set
Index = randperm(length(X1),3000);

T_index = Index(1:1000);
X1_train = X1(T_index);
X2_train = X2(T_index);
...
% vq = F(x1q,x2q);
% 
% plot3(X1_train,X2_train,T_train,'.',x1q,x2q,vq,'.'), grid on
...
% mesh(X1_train,X2_train,T_train)

% validation set
V_index = Index(1001:2000);
X1_val = X1(V_index);
X2_val = X2(V_index);
X_val = [X1_val, X2_val]; 
T_val = Tnew(V_index);
...
% 2. Build and train your feedforward Neural Network: use the training and validation sets. Build the ANN with 2
